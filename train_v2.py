...
for n_c, (f1, f2) in enumerate(combs):
    ...
    #print('current feature %60s %4d in %5.1f' % (name, n_c + 1, (time.time() - start) / 60))
    #print('\r' * 75, end='')
    ...
    # Label Encode
    lbl = LabelEncoder()
    lbl.fit(list(train_df[name].values) + list(test_df[name].values))
    train_df[name] = lbl.transform(list(train_df[name].values))
    test_df[name] = lbl.transform(list(test_df[name].values))
...
print('x test shape: ', test_df.shape)
